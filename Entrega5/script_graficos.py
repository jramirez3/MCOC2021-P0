...
fig.suptitle("Caso Matriz Llena")


while NCorrida <= 10:

	file = open("corrida N°{:0>2d} matriz llena.txt".format(NCorrida))
	Ns = []
	Tdes = []
	Tdss = []
	...
	for linea in file:
		...
		if int(Ni[0]) == 2:
			if min_tde < float(Ni[1]):
				min_tde = float(Ni[1])
			if min_tds < float(Ni[2]):
				min_tds = float(Ni[2])
...
fig = plt.figure("Matriz dispersa")
spec2 = gridspec.GridSpec(ncols=1, nrows=2, figure=fig)
f_ax1 = fig.add_subplot(spec2[0, 0])
f_ax2 = fig.add_subplot(spec2[1, 0])
...
f_ax2.set_ylim([10**-6, 100])
fig.suptitle("Caso Matriz dispersa")


while NCorrida <= 10:

	file = open("corrida N°{:0>2d} matriz dispersa.txt".format(NCorrida))
	Ns = []
	Tdes = []
	Tdss = []
